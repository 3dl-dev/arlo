#!/usr/bin/env python3
"""arlo's grounded translation layer: intent -> the command to run.

This is the half that makes arlo more than a list of commands. It takes the
...
How it stays grounded while still bridging vocabulary:
  ...
  - The candidates are capability CARDS (cards.py), each a real command paired
    ...
    arlo cannot invent a command; the worst it can do is pick the wrong real
  ...
  - The embedder is frontier-independent: a small local model provisioned at
    setup time (see provision). If confidence is below a floor, arlo says it
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="arlo: translate intent to a grounded command")
    ap.add_argument("cards", help="a cards.json from cards.py")
    ap.add_argument("query", nargs="+")
    ap.add_argument("--top", type=int, default=3)
    ap.add_argument("--floor", type=float, default=0.2,
                    help="below this confidence, arlo says it has no match rather than guess")
    ...
    if not hits or hits[0]["score"] < args.floor:
        print("arlo: no confident grounded match. It will not guess.")
        if hits:
            print(f"  (closest was {hits[0]['id']} at {hits[0]['score']}; this task has "
                  "no card yet -- harvest a runbook entry for it and rebuild the cards.)")
        return 1
